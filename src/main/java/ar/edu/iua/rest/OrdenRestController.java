...
public class OrdenRestController extends BaseRestController {
    ...
    public ResponseEntity<Orden> updateSurtidor(@RequestBody OrdenSurtidorDTO ordenSurtidorDTO) {
        Orden p = null;
        try {
            p = ordenBusiness.updateSurtidor(ordenSurtidorDTO);
    ...
    public ResponseEntity<Orden> updatePesajeInicial(@RequestBody PesajeDTO pesajeDTO) {
        Orden p = null;
        try {
            p = ordenBusiness.updatePesajeInicial(pesajeDTO);
            return new ResponseEntity<Orden>(p, HttpStatus.OK);
        } catch (BusinessException e) {
            log.error(e.getMessage(), e);
            return new ResponseEntity<Orden>(HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    public ResponseEntity<Orden> cerrarOrden(@RequestBody OrdenSurtidorDTO orden) {
        Orden p = null;
        try {
            p = ordenBusiness.cerrarOrdenPorCodigoExterno(orden);
            return new ResponseEntity<Orden>(p, HttpStatus.OK);
        } catch (BusinessException e) {
            log.error(e.getMessage(), e);
            return new ResponseEntity<Orden>(HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    public ResponseEntity<Orden> updatePesajeFinal(@RequestBody PesajeDTO pesajeDTO) {
        Orden p = null;
        try {
            p = ordenBusiness.updatePesajeFinal(pesajeDTO);
            return new ResponseEntity<Orden>(p, HttpStatus.OK);
        } catch (BusinessException e) {
            log.error(e.getMessage(), e);
            return new ResponseEntity<Orden>(HttpStatus.INTERNAL_SERVER_ERROR);
